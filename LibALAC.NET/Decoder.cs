...
namespace LibALAC
{
    ...
    public class Decoder : IDisposable
    {
        ...
        [DllImport("LibALAC32.dll", EntryPoint = "InitializeDecoderWithCookie", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr InitializeDecoderWithCookie32(byte[] inMagicCookie, int inMagicCookieSizet);
        [DllImport("LibALAC64.dll", EntryPoint = "InitializeDecoderWithCookie", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr InitializeDecoderWithCookie64(byte[] inMagicCookie, int inMagicCookieSize);
        ...
        /// <summary>
        ///    Default size of one PCM output packet.
        /// </summary>
        ...
        public Decoder(byte[] inMagicCookie)
        {
            intPtr = Is64BitProcess ? InitializeDecoderWithCookie64(inMagicCookie, inMagicCookie.Length) : InitializeDecoderWithCookie32(inMagicCookie, inMagicCookie.Length);
            if (intPtr == null)
                throw new LibALACException("InitializeDecoderWithCookie failed.");
            sampleRate = 0;
            channels = 0;
            bitsPerSample = 0;
            framesPerPacket = 0;
